...
- Generic version "2.3" → Search and present all 2.3.x options
- Product unclear → Ask: "Which product?" or "Which component?"
...
**Example Interaction:**
```
User: "What's new in version 2.3?"

❌ BAD (old way):
- Assumes which 2.3.x version
- Searches without confirming
- May miss what user wanted

✅ GOOD (new way):
"I'll help you find what's new in version 2.3! Let me check which exact versions are available..."

[Searches: search_filenames_fuzzy("2.3", limit=100)]

"I found these versions in the documentation:

**Product A 2.3.x:**
1. Product_A_Release_Notes_v2.3.1_Final
2. Product_A_Release_Notes_v2.3.4_Final
3. Product_A_Release_Notes_v2.3.7_Final

**Product B 2.3.x:**
4. Product_B_Release_Notes_v2.3.2_GA
5. Product_B_Release_Notes_v2.3.5_GA
6. Product_B_Release_Notes_v2.3.6_GA
...
```
User asks: "What's the latest version and when was it released (GA releases only)?"

ITERATION 1 (Filename search - CHEAP):
- search_filenames_fuzzy("Product X", limit=100)
- Gets: 47 files
- Self-validate: "Did I get ALL Product X versions?"
- Decision: Good coverage

ITERATION 2 (Filename search - CHEAP):
- search_filenames_fuzzy("Product X GA release", limit=100)
...
- Identify latest: Product_X_Release_Notes_v3.2.5_GA
- Self-validate: "Is v3.2.5 definitely the latest?"
...
```
After identifying correct file: Product_X_Release_Notes_v3.2.5_GA

ITERATION 4 (Content search - EXPENSIVE):
- search_with_filename_filter(
    query="release date GA general availability version 3.2.5",
...
- search_with_filename_filter(
    query="GA general availability production ready 3.2.5",
    limit=3, context_window=2
  )
...
```
User asks: "What are the security fixes, new features, and known issues in v2.5?"

ITERATION 1 (Filename):
- search_filenames_fuzzy("version 2.5", limit=100)
...
```json
{
  "query": "Product X version 2.5",  // Product + version
...
- search_filenames_fuzzy("Product X general availability", limit=100)
...
│    → search_filenames_fuzzy("Product X", limit=100)              │
│    → Self-validate: "Did I capture all Product X files?"         │
│                                                                   │
...
│    → search_filenames_fuzzy("Product X GA availability", 100)    │
